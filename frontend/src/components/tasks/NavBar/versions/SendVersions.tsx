...
import { VersionSchema, type versionForm } from "@/utils/utils";
import { useShowSendVersStore } from "@/store/tasks/store";

type forms = "send" | "pac" | "res" | string;
const Descriptions = {
  send: "Enter a name for your version, then select the type you want to send",
...
export default function SendVersions() {
  /**
  ...
  return (
    ...
    >
      ...
      >
        ...
        {forms === "pac" && <SendPac versionName={data} />}
        {forms === "res" && <SendRes versionName={data} />}
      </div>
    </div>
  );
}
