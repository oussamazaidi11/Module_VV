...
export default function SendPac({ taskName }: Props) {
...
const payload = {
  taskMaterialName: taskName, // the name of your task material
  taskMaterialWithResource: false,
  taskMaterialWithPackage: true,
  taskID: taskID, // your existing task ID
  package: {
    packageName: data.projectName, // from your form
...
console.log(payload)
try{
  toast.loading("Updloading taskmatrial")
  const res=await createTaskMaterial(client.id,payload)
  toast.success("Uploaded with success")
  close();

}catch(error){
  console.log(error)
  toast.error("please insert a valid link",{duration:1000})
}
